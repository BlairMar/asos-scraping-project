...
class AsosScraper:
     ...
     def accept_cookies_button(self):
        time.sleep(4)
        click_accept_cookies = self.driver.find_element_by_xpath(
     ...
     def choose_category(self):
        category_button = self.driver.find_element_by_xpath(
            '//*[@id="chrome-sticky-header"]/div[2]/div[2]/nav/div/div/button[2]')
     ...
     def get_submenu_product_list(self):
         self.extract_links(
             '//*[@id="029c47b3-2111-43e9-9138-0d00ecf0b3db"]/div/div[2]/ul/li[1]/ul/li[*]')
         self.driver.get(self.links[0])

     # this method uses the "extract_links" method to extract the hrefs for every product in the 'New in/View all'
     def get_product_list_urls(self):
        self.extract_links(
            '//*[@id="plp"]/div/div/div[2]/div/div[1]/section[1]/article')
        # save the list with hrefs in another variable that will be reffered to in the following methods
        self.product_urls = self.links
            ...
            'Colour': []
               ...
               try:  # find details info
                  for key in self.xpath_dict:
                   dict_key = self.driver.find_element_by_xpath(
                       self.xpath_dict[key])
